...
export type UshQuestionGen = (rng: () => number, ctx: UshCtx, i: number) => ExamQuestion;

export function ushMc(
 rng: () => number,
 ctx: UshCtx,
 i: number,
 tag: string,
 ...
 let fig = figure;
 if (!fig && rng() < TEXT_ITEM_STIMULUS_PROBABILITY.ush) {
 fig = thematicStimulusUsh(rng, ctx.unitIndex);
 }
 const out = fig ? { ...base, figure: fig } : base;
 return { ...out, procedural_structure_id: proseMcqStructureId(tag, stem, correct) };
}

export function ushItem(
 tag: string,
 stem: string,
 correct: string,
 ...
 return (rng, ctx, i) => ushMc(rng, ctx, i, tag, stem, correct, w[0], w[1], w[2], explanation, figure);
}
